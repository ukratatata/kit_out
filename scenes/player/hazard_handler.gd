...
## How long the player's Z-lock is released after an off-track hit, allowing
## them to be knocked out of the lane before being eased back.
@export var off_track_duration: float = 0.5

var stun_timer: float      = 0.0
var iframe_timer: float    = 0.0
var off_track_timer: float = 0.0
...
func tick_timers(delta: float) -> void:
	...
	off_track_timer = maxf(off_track_timer - delta, 0.0)
...
func apply_hit_3d(knockback: Vector3) -> void:
	...
	stun_timer      = stun_duration
	iframe_timer    = iframe_duration
	off_track_timer = off_track_duration
	player.took_damage.emit()
	player.enter_stunned()
